...
HXLINE( 266)					_hx_tmp = (timer->time >= (int)0);
            				}
            				else {
...
HXLINE( 283)			while((loopedTimers->length > (int)0)){
...
HXDLIN( 311)		if ((index != (int)-1)) {
HXDLIN( 311)			array[index] = array->__get((array->length - (int)1)).StaticCast<  ::flixel::util::FlxTimer >();
...
HXDLIN( 323)				if ((timer->loops > (int)0)) {
...
HXLINE( 326)		{
HXLINE( 326)			int _g2 = (int)0;
HXDLIN( 326)			while((_g2 < timersToFinish->length)){
HXLINE( 326)				 ::flixel::util::FlxTimer timer1 = timersToFinish->__get(_g2).StaticCast<  ::flixel::util::FlxTimer >();
HXDLIN( 326)				_g2 = (_g2 + (int)1);
HXLINE( 328)				while(!(timer1->finished)){
HXLINE( 330)					timer1->update((timer1->time - timer1->_timeCounter));
HXLINE( 331)					timer1->onLoopFinished();
            				}
            			}
            		}
            	}
