...
HXLINE(  36)			return ::lime::utils::AssetManifest_obj::parse(bytes->getString((int)0,bytes->length,null()),rootPath);
            		}
            		else {
...
HXLINE(  77)		 ::Dynamic manifestData =  ::haxe::format::JsonParser_obj::__alloc( HX_CTX ,data)->parseRec();
...
HXLINE( 132)		if ((queryIndex > (int)-1)) {
HXLINE( 134)			basePath = path.substr((int)0,queryIndex);
            		}
            		else {
...
HXLINE( 145)			basePath = basePath.substr((int)0,(basePath.length - (int)1));
            		}
...
HXLINE( 171)		if ((queryIndex > (int)-1)) {
HXLINE( 173)			rootPath = path.substr((int)0,queryIndex);
            		}
            		else {
...
HXLINE( 185)			rootPath = rootPath.substr((int)0,(rootPath.length - (int)1));
            		}
